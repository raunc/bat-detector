...
// This function transmits and receives a byte. 
uint16_t SERCOM::transferDataSPI(uint8_t data)
{
	sercom->SPI.DATA.bit.DATA = data; // Initiate byte transfer.
     while(sercom->SPI.INTFLAG.bit.RXC == 0); // Wait for data to be available in the receive buffer.
     return sercom->SPI.DATA.bit.DATA; // Read received byte, then increment pointer into buffer. 

	// Returned value is actually an integer, which has some data in the high byte that should be stripped in the SPI lib.
}
